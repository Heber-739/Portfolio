...
export class CryptoFormComponent implements OnInit {
  ...
  select(item: Coin) {
    ...
    this.render.setProperty(
      this.minDeposit.nativeElement,
      'innerHTML',
      `Deposite un mínimo de ${this.minNow.toFixed(8)}`
    );
    this.mySetInterval = setInterval(() => {
      this.http
        .get<SimpleCoin>(
    ...
    this.mySetInterval = setInterval(() => {}, 15000);
  }
  ...
  cryptoSelect(ev: string) {
    let newArratCrypto = this.cryptoList.filter(
      (el) => el.name.toUpperCase().includes(ev.toUpperCase()) && ev != ''
    );
